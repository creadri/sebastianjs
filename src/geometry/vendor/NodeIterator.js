// Vendored from svgdom@0.1.28 (src/utils/NodeIterator.js) — MIT.
...
const showThisNode = (whatToShow, node) => {
  ...
  if (
    ...
    return true
  if (whatToShow & NodeFilter.SHOW_TEXT && node.nodeType === node.TEXT_NODE)
    return true
  if (
    whatToShow & NodeFilter.SHOW_ENTITY_REFERENCE &&
    node.nodeType === node.ENTITY_REFERENCE_NODE
  )
    return true
  if (whatToShow & NodeFilter.SHOW_ENTITY && node.nodeType === node.ENTITY_NODE)
    return true
  if (
    whatToShow & NodeFilter.SHOW_PROCESSING_INSTRUCTION &&
  ...
  if (
    ...
    return true
  if (
    whatToShow & NodeFilter.SHOW_DOCUMENT_TYPE &&
  ...
  if (
    ...
    return true
  if (
    whatToShow & NodeFilter.SHOW_NOTATION &&
